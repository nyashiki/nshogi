...
nshogi_piece_t positionApiPieceOn(const nshogi_position_t* CPosition,
                                  nshogi_square_t CSquare) {
    const core::Position* Position =
        reinterpret_cast<const core::Position*>(CPosition);
...
uint8_t positionApiGetStandCount(const nshogi_position_t* CPosition,
                                 nshogi_color_t CColor,
                                 nshogi_piece_type_t CPieceType) {
    const core::Position* Position =
        reinterpret_cast<const core::Position*>(CPosition);
    core::Color Color = static_cast<core::Color>(CColor);
    core::PieceTypeKind PieceType =
        static_cast<core::PieceTypeKind>(CPieceType);
