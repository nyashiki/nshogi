...
PYBIND11_MODULE(nshogi, Module) {
    ...
    pybind11::class_<nshogi::ml::SimpleTeacher>(MLModule, "SimpleTeacher")
        ...
        .def("check", [](const nshogi::ml::SimpleTeacher& T) {
            const auto State = T.getState();
            nshogi::core::internal::ImmutableStateAdapter Adapter(State);
            return Adapter->getCheckerBB().isZero() ? 0.0f : 1.0f;
            })
        .def("declaration_score",
             [](const nshogi::ml::SimpleTeacher& T) {
                const auto State = T.getState();
                nshogi::core::internal::ImmutableStateAdapter Adapter(State);
                ...
                if (State.getSideToMove() == nshogi::core::Black) {
                    const float MyScore =
                            (float)Adapter->computeDeclarationScore<nshogi::core::Black>() / 28.0f;
                    const float OpScore =
                            (float)Adapter->computeDeclarationScore<nshogi::core::White>() / 27.0f;
