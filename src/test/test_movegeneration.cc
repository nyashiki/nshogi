...
void testMoveGenerationNum(const std::string& Sfen, size_t Count) {
    ...
    const auto Moves =
        nshogi::core::MoveGenerator::generateLegalMoves<WilyPromote>(State);
...
TEST(MoveGeneration, FindMoves1) {
    const std::string Sfens[] = {
        "1g1f", "2g2f", "3g3f", "4g4f", "5g5f", "6g6f", "7g7f", "8g8f",
        "9g9f", "3i3h", "3i4h", "7i6h", "7i7h", "4i3h", "4i4h", "4i5h",
        "6i5h", "6i6h", "6i7h", "5i4h", "5i5h", "5i6h", "1i1h", "9i9h",
    ...
    for (const auto& Sfen : Sfens) {
        nshogi::core::Move32 Move =
            nshogi::io::sfen::sfenToMove32(State.getPosition(), Sfen);
        TEST_ASSERT_TRUE(Moves.find(Move) != Moves.end());
    }

    nshogi::core::Move32 IllegalMove =
        nshogi::io::sfen::sfenToMove32(State.getPosition(), "G*5e");
...
TEST(MoveGeneration, ExamplePositions) {
    ...
    while (std::getline(Ifs, Line)) {
        ...
        for (uint16_t I = 0; I < NumTestRandomMove; ++I) {
            ...
            TEST_ASSERT_TRUE(GeneratedMoves.find(RandomLegalMove) !=
                             GeneratedMoves.end());
        }
    }
}
...
TEST(MoveGeneration, CheckPositions) {
    ...
    while (std::getline(Ifs, Line)) {
        ...
        for (uint16_t I = 0; I < NumMoves; ++I) {
            ...
            if (!Move.drop() && Move.pieceType() == nshogi::core::PTK_Pawn &&
                !Move.promote()) {
                auto PromotableBB =
                    (State.getPosition().sideToMove() == nshogi::core::Black)
                        ? nshogi::core::internal::bitboard::PromotableBB
                              [nshogi::core::Black]
                        : nshogi::core::internal::bitboard::PromotableBB
                              [nshogi::core::White];
            ...
            if (!Move.drop() && Move.pieceType() == nshogi::core::PTK_Lance &&
                !Move.promote()) {
                auto FirstAndSecondFurthestBB =
                    (State.getPosition().sideToMove() == nshogi::core::Black)
                        ? nshogi::core::internal::bitboard::
                              FirstAndSecondFurthestBB[nshogi::core::Black]
                        : nshogi::core::internal::bitboard::
                              FirstAndSecondFurthestBB[nshogi::core::White];
        ...
        for (uint16_t I = 0; I < LegalMoves.size(); ++I) {
            TEST_ASSERT_TRUE(GeneratedMoves.find(LegalMoves[I]) !=
                             GeneratedMoves.end());
        }
    }
}

TEST(MoveGeneration, CaptureMoves) {
    const int N = 1000;
    std::mt19937_64 mt(20260426);
    ...
    for (int I = 0; I < N; ++I) {
        ...
        for (uint16_t Ply = 0; Ply < 512; ++Ply) {
            ...
            std::set<nshogi::core::Move32> CaptureMoveSet;
            for (const auto& Move : Moves) {
                if (Move.capturePieceType() != nshogi::core::PTK_Empty) {
                    CaptureMoveSet.insert(Move);
                }
            }

            std::set<nshogi::core::Move32> CaptureMoveSet2;
            const auto CaptureMoves =
                nshogi::core::MoveGenerator::generateLegalCaptureMoves(State);
            for (const auto& CaptureMove : CaptureMoves) {
                CaptureMoveSet2.insert(CaptureMove);
            }

            // Check all capture moves are generated by `generateLegalCaptureMoves()`.
            for (const auto& CaptureMove : CaptureMoveSet) {
                TEST_ASSERT_TRUE(CaptureMoveSet2.find(CaptureMove) !=
                                 CaptureMoveSet2.end());
            }
