...
bool testDiffHuffmanCodeOneMoveAfter(const nshogi::core::Position& Pos) {
    ...
    nshogi::core::HuffmanCode OriginalHC =
        nshogi::core::HuffmanCode::encode(S.getPosition());
...
TEST(Huffman, AllStandsBlack) {
    nshogi::core::Position Pos = nshogi::io::sfen::PositionBuilder::newPosition(
...
TEST(Huffman, Memcpy) {
    ...
    for (int I = 0; I < N; ++I) {
        ...
        for (int Ply = 0; Ply < 1024; ++Ply) {
            ...
            const auto HC2 = nshogi::core::HuffmanCode(Codes[3], Codes[2],
                                                       Codes[1], Codes[0]);
