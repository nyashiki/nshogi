...
int main() {
    ...
    //             State.getBitboard<core::Black,
    //             core::PTK_Rook>().forEach([&](core::Square Sq) {
    //                 core::bitboard::Bitboard RookAttackBB =
    //                 core::bitboard::getRookAttackBB<nshogi::core::PTK_Rook>(Sq,
    //                 OccupiedBB);

    //                 auto Bottom = RankBB[core::RankA].subtract(SquareBB[Sq]);
    //                 auto Top = SquareBB[Sq].subtract(OccupiedBB ^
    //                 SquareBB[Sq]);

    //                 auto BB = Top.subtract(Bottom) ^ SquareBB[Sq];

    //                 if ((RookAttackBB & FileBB[core::squareToFile(Sq)]) !=
    //                 BB) {
...

    {
        ...
        runCountBench("Movegeneration initial position", benchMoveGeneration, N,
                      State);
    }

    {
        nshogi::core::State State = nshogi::io::sfen::StateBuilder::newState(
            ...
            "RGgsn5p 1");
        runCountBench("Movegeneration matsuri position", benchMoveGeneration, N,
                      State);
    }
    ...
    //     nshogi::core::State State =
    //     nshogi::io::sfen::StateBuilder::getInitialState();
    //     runCountBench("Movegeneration initial position",
    //     benchMoveGenerationInternal, N,
    //     *core::internal::ImmutableStateAdapter(State).get());
    // }

    // {
    //     nshogi::core::State State =
    //     nshogi::io::sfen::StateBuilder::newState("l6nl/5+P1gk/2np1S3/p1p4Pp/3P2Sp1/1PPb2P1P/P5GS1/R8/LN4bKL
    //     w RGgsn5p 1"); runCountBench("Movegeneration matsuri position",
    //     benchMoveGenerationInternal, N,
    //     *core::internal::ImmutableStateAdapter(State).get());
    // }
    ...
    // {
    //     nshogi::core::State State =
    //     nshogi::io::sfen::StateBuilder::newState("R8/2K1S1SSk/4B4/9/9/9/9/9/1L1L1L3
    //     b RBGSNLP3g3n17p 1"); runCountBench("Movegeneration max-moves#
    //     position", benchMoveGenerationInternal, N,
    //     *core::internal::ImmutableStateAdapter(State).get());
    // }

    {
        ...
        while (std::getline(Ifs, Line)) {
            PositiveStates.push_back(
                nshogi::io::sfen::StateBuilder::newState(Line));
        }

        runCountBench("Positive Mate1Ply", benchMate1ply, 10000,
                      PositiveStates);
    }

    {
        ...
        std::string Line;
        while (std::getline(Ifs, Line)) {
            NegativeStates.push_back(
                nshogi::io::sfen::StateBuilder::newState(Line));
        }

        runCountBench("Negative Mate1Ply", benchMate1ply, 10000,
                      NegativeStates);
    }
